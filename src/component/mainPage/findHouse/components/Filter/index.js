...
export default class Filter extends React.Component {
  ...
  // 初始化默认选择项
  initDefaultData() {
    const { gotPickerValue: defaultData, pickerData: data, openType: type } = this.state
    ...
    if (type === 'area') {
      ...
      if (gotPickerValue[0] !== undefined) {
        if (gotPickerValue[0].length === 3) {
          updatedValue = gotPickerValue[0][2] !== 'null' ? gotPickerValue[0][2] : gotPickerValue[0][1]
        }
      }
    } else {
      let index = this.getIndexByType(type)
      if (gotPickerValue[index] !== undefined && gotPickerValue[index].length !== 0) {
  ...
  renderFilterMore() {
    ...
    return (
      <Spring
        ...
        to={{
          transform: openType !== 'select' ? 'translate(100%,0%)' : 'translate(0%,0%)',
        }}
        onStart={() => {
        ...
        {(props) => {
          return (
            <animated.div style={props} className={styles.filterMoreBox}>
              <FilterMore defaultData={this.state.gotPickerValue[3]} sendValue={this.getValue} data={data} />
            </animated.div>
          )
        }}
      </Spring>
    ...
    return (
      <>
        ...
        {openType === 'area' || openType === 'way' || openType === 'money' ? (
          <div>
            <FilterPicker data={this.state.pickerData} type={this.state.openType} sendValue={this.getValue} defaultList={this.state.defaultList} defaultData={this.state.defaultData} />
            {/* 取消，确定按钮 */}
